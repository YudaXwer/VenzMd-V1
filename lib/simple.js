...
react = async (conn, options = {}) {
    if (!conn) throw new Error("sock is not defined")
    if (conn.type == "legacy") throw new Error("LegacySocket not yet supported")
    ...
    let react = await proto.ReactionMessage.create({
     ...
     if (sock) return await conn.relayMessage(react.key.remoteJid, {
